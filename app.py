...
def get_duplicate_origin(duplicate_id):
    """Get the original request number for a duplicate"""
    try:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
...
# ============================================================
# Dashboard Page - Simple & Clean
# ============================================================

def dashboard_page():
    """Simple dashboard with duplicate detection"""
        ...
        with st.spinner("Searching for links..."):
            ...
            if result and result.get("success"):
                ...
                if is_dup:
                    ...
                    if dup_of:
                        orig_req = get_duplicate_origin(dup_of)
                        if orig_req:
                            st.caption(f"First seen in Request #{orig_req}")
                else:
                    st.success("✅ This is a FRESH NEW link")
            else:
                msg = result.get("message", "Failed to get a valid link") if result else "Failed to get a valid link"
                st.error(msg)
